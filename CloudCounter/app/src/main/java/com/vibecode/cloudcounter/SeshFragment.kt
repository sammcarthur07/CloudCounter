...
import com.vibecode.cloudcounter.databinding.FragmentSeshBinding
...
class SeshFragment : Fragment() {
    ...
    private fun createSmokerStatsRowsFixed(stat: PerSmokerStats) {
        ...
        // Calculate time since last activity for each type
    ...
    // Called when session is ended from History or elsewhere
    fun onSessionEnded() {
        Log.d("SeshFragment", "📋 onSessionEnded called - clearing session state")
        ...
        _binding?.let { binding ->
            binding.textRoomName.text = "No Active Session"
            binding.textRoomCode.visibility = View.GONE
            binding.textThisSesh.text = "00:00"
            binding.perSmokerStatsContainer.removeAllViews()
            binding.fabResumeSesh.visibility = View.GONE
        ...
        // Force the ViewModel to clear its state
        sessionStatsVM.endSession()
    }
    
    // Called to refresh stats when activities are deleted
    fun refreshStats() {
        Log.d("SeshFragment", "📋 refreshStats called - requesting full stats refresh")
        
        // Get MainActivity instance and trigger its refresh method with forceRefresh
        val mainActivity = activity as? MainActivity
        if (mainActivity != null) {
            Log.d("SeshFragment", "📋 Calling MainActivity.refreshLocalSessionStatsIfNeeded(forceRefresh = true)")
            mainActivity.refreshLocalSessionStatsIfNeeded(forceRefresh = true)
        } else {
            Log.d("SeshFragment", "📋 MainActivity not available, using ViewModel refresh")
            // Fallback to ViewModel refresh
            sessionStatsVM.recalculateGaps()
            sessionStatsVM.forceStatsRefresh()
        }
    }
}
