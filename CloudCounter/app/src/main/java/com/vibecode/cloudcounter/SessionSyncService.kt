...
class SessionSyncService(
    ...
    suspend fun removeActivityFromRoom(
        ...
        return try {
            ...
            firestore.runTransaction { transaction ->
                ...
                Log.d(TAG, "Looking for activity to remove: smokerUid=$smokerUid, type=${activityType.name}, timestamp=$timestamp")
                Log.d(TAG, "Current room has ${activities.size} activities:")
                activities.forEach { activity ->
                    Log.d(TAG, "  Room activity: smokerId=${activity.smokerId}, type=${activity.type}, timestamp=${activity.timestamp}")
                }
                
                val activityToRemove = activities.find { activity ->
                    val matches = activity.smokerId == smokerUid &&
                            activity.type.equals(activityType.name, ignoreCase = true) &&
                            activity.timestamp == timestamp
                    if (!matches && activity.timestamp == timestamp) {
                        Log.d(TAG, "Found activity with same timestamp but different details:")
                        Log.d(TAG, "  - smokerId: ${activity.smokerId} (looking for: $smokerUid)")
                        Log.d(TAG, "  - type: ${activity.type} (looking for: ${activityType.name})")
                    }
                    matches
                }
                ...
                if (activityToRemove != null) {
                    ...
                    Log.d(TAG, "Found and removing activity from room. Activities before: ${room.safeActivities().size}, after: ${activities.size}")
                    ...
                    Log.d(TAG, "✅ REMOVED activity: $activityType for $smokerUid at $timestamp. Room now has ${activities.size} activities")
                } else {
                    Log.e(TAG, "❌ FAILED TO FIND activity for removal: $activityType for $smokerUid at $timestamp")
                    Log.e(TAG, "❌ Room has ${activities.size} activities but none matched")
                }
            }.await()
    ...
    suspend fun addActivityToRoom(
        ...
        // Check if this activity is in the permanent block list
        val blockedPrefs = context.getSharedPreferences("blocked_activities", Context.MODE_PRIVATE)
        val activityKey = "${smokerUid}_${activityType}_${timestamp}"
        if (blockedPrefs.getBoolean(activityKey, false)) {
            Log.d(TAG, "🚫 Activity is in permanent block list, skipping cloud sync: $activityKey")
